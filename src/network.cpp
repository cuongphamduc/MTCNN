...
void convolution(const Weight *weight, const pBox *pbox, pBox *outpBox, const struct pBox *matrix) {
    ...
    if (weight->pad == 0) {
        ...
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, weight->selfChannel, matrix->height, matrix->width, 1,
                    weight->pdata, matrix->width, matrix->pdata, matrix->width, 0, outpBox->pdata, matrix->height);
    } else {
...
void fullconnect(const Weight *weight, const pBox *pbox, pBox *outpBox) {
    ...
    cblas_sgemv(CblasRowMajor, CblasNoTrans, weight->selfChannel, weight->lastChannel, 1, weight->pdata,
                weight->lastChannel, pbox->pdata, 1, 0, outpBox->pdata, 1);
}
